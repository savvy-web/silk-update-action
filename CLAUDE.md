...
- **Domain services**: `BranchManager`, `PnpmUpgrade`, `ConfigDeps`,
  ...
  (`^2.0.0`) directly (no local `Workspaces` Tag), still consumed by
...
- **Changesets adapter**: `services/changesets.ts` is a thin adapter over
  `Changesets.DepsRegen` from `@savvy-web/silk-effects` (`^2.0.0`), which is the
...
**Currently active:** a dogfood link IS active for `@savvy-web/silk-effects`. `pnpm-workspace.yaml` has an `overrides` block linking it to a local prod dist of the unreleased `2.0.0` (`link:/Users/spencer/workspaces/savvy-web/systems/packages/silk-effects/dist/prod/npm/pkg`), and both the `@savvy-web/silk-effects` and `workspaces-effect` ranges are bumped to `^2.0.0` (this is where `Changesets.DepsRegen` lands). `workspaces-effect@2.0.0` is already published on the registry; silk-effects `2.0.0` is pending release, so the link flips to the registry `^2.0.0` (drop the `overrides` block, `pnpm install`) once silk-effects `2.0.0` publishes. The other first-party deps remain unlinked at their registry versions (`runtime-resolver@^0.3.10`, `semver-effect@^0.2.1`, `@savvy-web/github-action-effects@^2.0.0`).
